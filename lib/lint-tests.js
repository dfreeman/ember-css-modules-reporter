...
module.exports = class LintTests extends Plugin {
  ...
  build() {
    ...
    let tests = Array.from(messages.keys(), (file) => {
      ...
      let issues = fileMessages.map(message => `${message.line}:${message.column} - ${message.text}`);
      return {
        name: file,
    ...
    let testPath = path.join(this.outputPath, 'css-warnings.lint-test.js');
    let content = this.project.generateTestFile(`CSS | ${this.type}`, tests);
